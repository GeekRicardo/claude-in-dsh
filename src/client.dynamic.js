...
return {
  ...
  apply(ctx) {
    ...
    styles.insert(`
      ...
      .ccmode-ring { display:inline-flex; align-items:center; gap:5px; color:var(--dsw-alias-label-secondary, inherit); }
      .ccmode-ring-label { font-size:11px; font-variant-numeric:tabular-nums; }
    ...
    // ---------- context ring, under the send button ----------
    //
    // waku shows how much of the model's context window the conversation
    // occupies; the same number is in Claude's stream (prompt + cache + output
    // of the last call, over the model's contextWindow), so the composer can
    // carry it as a small ring rather than a number nobody reads.
    function ContextRing(props) {
      ...
      React.useEffect(() => { load(sessionId) }, [sessionId])
      if (state.mode !== 'claude') return null
      const window = Number(state.contextWindow || 0)
      ...
      const tone = percent >= 90 ? 'ccmode-ring-crit' : (percent >= 70 ? 'ccmode-ring-warn' : 'ccmode-ring-ok')
      const compact = (value) => (value >= 1000000
        ? (value / 1000000).toFixed(value >= 10000000 ? 0 : 1) + 'M'
        : (value >= 1000 ? Math.round(value / 1000) + 'k' : String(value)))

      return h('span', {
        className: 'ccmode-ring',
        title: '上下文占用 ' + percent + '%（' + compact(tokens) + ' / ' + compact(window) + ' tokens）',
      },
        h('svg', { width: size, height: size, viewBox: '0 0 ' + size + ' ' + size, 'aria-hidden': true },
          h('circle', {
            cx: size / 2, cy: size / 2, r: radius, fill: 'none',
            stroke: 'currentColor', strokeOpacity: 0.18, strokeWidth: stroke,
          }),
          h('circle', {
            className: tone,
            cx: size / 2, cy: size / 2, r: radius, fill: 'none',
        ...
        h('span', { className: 'ccmode-ring-label ' + tone }, percent + '%'))
    }
    ...
    function startBadges() {
      ...
      if (typeof MutationObserver === 'function') {
        badgeObserver = new MutationObserver(() => { paintBadges(); safelyClient('menu item', injectImportItem) })
        badgeObserver.observe(document.body, { childList: true, subtree: true })
      }
      document.addEventListener('click', noteWorkspaceClick, true)
      const stopMenuWatch = ctx.interval(() => { safelyClient('menu item', injectImportItem) }, 300)
      return () => {
        stopMenuWatch()
        stopPull()
    ...
    function noteWorkspaceClick(event) {
      safelyClient('workspace click', () => {
        ...
        for (let depth = 0; node !== null && node !== undefined && depth < 10; depth += 1) {
          if (typeof node.className === 'string' && node.className.indexOf(PROJECT_ROW_CLASS) >= 0) {
    ...
    function injectImportItem() {
      ...
      for (let index = 0; index < menus.length; index += 1) {
        ...
        clone.classList.add(IMPORT_ITEM_CLASS)
        // Reuse dsh's own item markup so the entry looks native; just retext it.
        ...
        for (let at = 0; at < labels.length; at += 1) {
          if (labels[at].children.length === 0 && String(labels[at].textContent).trim().length > 0) {
            labels[at].textContent = '导入 Claude Code 对话'
            retexted = true
            break
          }
        }
        if (!retexted) clone.textContent = '导入 Claude Code 对话'
        const svg = clone.querySelector('svg')
        if (svg !== null && svg.parentElement !== null) svg.parentElement.textContent = '✳'
    ...
    slots.inject('conversation.composer.dock', () => slots.register(
      ...
      (props) => h(ContextRing, { sessionId: props && props.sessionId }),
    ))
    ...
    console.log('cc-mode: client v70 ready — native chrome, ' + Object.keys(TOOL_SUMMARY).length + ' tool rows')
  },
}
