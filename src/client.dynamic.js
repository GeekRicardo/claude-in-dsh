...
return {
  ...
  apply(ctx) {
    ...
    function openImportPreview(sessionId, cwd, claudeSessionId, workspace) {
      ...
      const head = el('div', 'ccmode-import-head')
      head.appendChild(el('span', 'ccmode-import-title', '导入 Claude Code 对话'))
    ...
    function previewMessage(item) {
      ...
      line.appendChild(el('span', 'ccmode-import-role', item.role === 'user' ? '你' : 'Claude'))
      line.appendChild(el('div', 'ccmode-import-text', item.text))
      return line
    }
    ...
    function showPreview(scrim, body, foot, note, sessionId, cwd, row, workspace) {
      body.textContent = ''
      body.appendChild(el('div', 'ccmode-import-empty', '读取中…'))
      note.textContent = '导入后，会在这个工作区新建会话并接着这段 Claude 对话继续。'
      ...
      importButton.onclick = () => adoptConversation(sessionId, cwd, row.claudeSessionId, workspace)
    ...
    function adoptConversation(sessionId, cwd, claudeSessionId, workspace) {
      ...
      if (sessions === undefined || typeof sessions.create !== 'function') {
        console.error('cc-mode: cannot create a conversation to import into')
        return undefined
      }
      ...
      return sessions.create(target)
        ...
        .catch((error) => console.error('cc-mode: import failed:', error))
    }
