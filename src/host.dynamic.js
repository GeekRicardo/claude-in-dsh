...
return {
  ...
  apply(ctx) {
    ...
    function rebindServices() {
      ...
      serviceRebinds += 1
      if (serviceRebinds <= 10) {
        // Re-arming from a timer callback: if the plugin is being disposed
        // right now the context is inactive and this would throw into the
        // runtime — stopping the retries is the correct outcome then.
        try { ctx.timeout(rebindServices, 2000) } catch (error) { /* disposing */ }
    ...
    ctx.timeout(rebindServices, 2000)
