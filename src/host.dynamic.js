...
return {
  ...
  apply(ctx) {
    ...
    async function ensureRun(sessionId, cwd, signal) {
      ...
      if (live !== undefined && !live.closed && live.snapshot === snapshot) return live
    ...
    const disposers = [
      ...
      harness.handle('claude.adopt', (args) => {
        const sessionId = String(args.sessionId || '')
        ...
        return { ok: true, state: publicState(sessionId) }
      }),
    ...
    console.log('cc-mode: host v57 ready — approval bridge', approval === undefined ? 'off' : 'on')
  },
}
