...
return {
  ...
  apply(ctx) {
    ...
    async function probeContextWindow(sessionId, cwd) {
      const state = stateOf(sessionId)
      const key = contextWindowKeyOf(state)
      if (state.contextWindowKey === key && Number(state.contextWindow) > 0) return
      if (windowProbes.has(sessionId) || activeTurns.has(sessionId)) return
      if (windowProbeFailures.has(sessionId + '|' + key)) return
      windowProbes.add(sessionId)
      try {
        const work = await runCommandForPlugin(sessionId, cwd, '/context')
        if (work === undefined) return
        const answer = await work
        const found = parseContextWindow(answer === undefined ? '' : answer.text)
    ...
    async function driveTurn(agent, messages, turn, signal) {
      ...
      try {
        ...
        wakeForProjection(sessionId)
        if (run.cwd) probeContextWindow(sessionId, run.cwd).catch(() => undefined)
            ...
            if (init !== undefined && typeof init.model === 'string' && init.model.length > 0) {
              ...
              if (typeof cwd === 'string' && cwd.length > 0) {
                ...
                if (model !== undefined && model.length > 0) {
                  state.route = model
                  const bare = model.replace(/\[[^\]]*\]$/, '')
                  if (state.model.length === 0 && MODELS.some((entry) => entry.id === bare)) state.model = bare
                  changed = true
                }
    ...
    console.log('cc-mode: host v99 ready — approval bridge', approval === undefined ? 'off' : 'on')
  },
}
